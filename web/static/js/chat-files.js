...
let chatFilesCache = [];
/** GET /api/chat-uploads backdirectorypath(Files), files */
...
/** Filescurrentpath( chat_uploads ), ['2024-03-21','uuid'] */
let chatFilesBrowsePath = [];
/** ,Filespath(chat_uploads directory), 2026-03-21/uuid/sub */
let chatFilesPendingUploadDir = '';
/** File Managementpage,disabled */
...
async function loadChatFilesPage() {
 ...
 wrap.innerHTML = '<div class="loading-spinner" data-i18n="common.loading">load…</div>';
 ...
 wrap.classList.remove('chat-files-table-wrap--grouped');
 wrap.classList.remove('chat-files-table-wrap--tree');
 const msg = (typeof window.t === 'function') ? window.t('chatFilesPage.errorLoad') : 'Failed to load';
...
async function copyChatFilePathIdx(idx) {
 ...
 if (ok) {
 const msg = (typeof window.t === 'function') ? window.t('chatFilesPage.pathCopied') : 'Path copied — paste it into chat';
...
function chatFilesEditBlockedHint() {
 ...
 : 'Binary files (images, archives, etc.) cannot be edited as text here. Use Download and open locally.';
}
...
function chatFilesAlertMessage(raw) {
 ...
 return (typeof window.t === 'function') ? window.t('chatFilesPage.editTooLarge') : 'File exceeds 2MB and cannot be edited here. Download and edit locally.';
 }
 return s || ((typeof window.t === 'function') ? window.t('chatFilesPage.errorGeneric') : 'Something went wrong. Please try again.');
}
...
function renderChatFilesTable() {
 ...
 const emptyMsg = (typeof window.t === 'function') ? window.t('chatFilesPage.empty') : 'No chat uploads yet';
 ...
 const thSubPath = (typeof window.t === 'function') ? window.t('chatFilesPage.colSubPath') : 'Subfolder';
 const thName = (typeof window.t === 'function') ? window.t('chatFilesPage.colName') : 'Name';
 ...
 const tCopyTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.copyPathTitle') : 'Copy the absolute path on the server; paste into chat to reference this file');
 const tDlTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.download') : 'Download');
 const tMoreTitle = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.moreActions') : 'More: open chat, edit, rename, delete');
 ...
 const tCopyFolder = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.copyFolderPathTitle') : 'Copy relative path under chat_uploads/…');
 const tEnter = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.enterFolderTitle') : 'Open folder');
 ...
 const tUploadToFolder = escapeHtml((typeof window.t === 'function') ? window.t('chatFilesPage.uploadToFolderTitle') : 'Upload file into this folder');
...
async function deleteChatFolderFromBrowse(folderName) {
 ...
 const rel = segs.join('/');
 const q = (typeof window.t === 'function') ? window.t('chatFilesPage.confirmDeleteFolder') : 'Delete this folder and everything inside it? This cannot be undone.';
 if (!confirm(q)) return;
 try {
 ...
 : 'That folder is not on the server anymore; list refreshed.';
...
async function copyChatFolderPathFromBrowse(folderName) {
 ...
 const ok = await chatFilesCopyText(text);
 if (ok) {
 const msg = (typeof window.t === 'function') ? window.t('chatFilesPage.folderPathCopied') : 'Folder path copied — paste into chat if needed';
 chatFilesShowToast(msg);
 } else {
 const fail = (typeof window.t === 'function') ? window.t('common.copyFailed') : 'Copy failed';
 alert(fail);
...
async function submitChatFilesMkdir() {
 ...
 : 'Invalid name: cannot be empty or contain /, \\, or use . or ..';
 alert(msg);
 return;
 }
...
async function onChatFilesUploadPick(ev) {
 ...
 : 'Uploaded. Use "Copy path" to copy the absolute path.';
 chatFilesShowToast(msg);
 }
 } catch (e) {
 alert((e && e.message) ? e.message : String(e));
