 ...
 * Returns a fetch-like object: ok, status, json(), text()
 */
        ...
        if (!response.ok || !result.token) {
            if (errorBox) {
                const fallback = (typeof window !== 'undefined' && typeof window.t === 'function')
                    ...
                    : 'Sign-in failed, please check the password';
...
async function bootstrapApp() {
    if (!isAppInitialized) {
        try {
            ...
            console.warn('Failed to wait for i18n readiness; continuing chat initialization', e);
        }
        initializeChatUI();
...
async function initializeApp() {
    ...
    if (hasStoredAuth && isTokenValid()) {
        try {
            ...
            console.warn('Local session is no longer valid. Please sign in again');
        }
    }
