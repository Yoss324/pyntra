...
func main() {
	...
	// Test startup (only test enabled servers)
	fmt.Println("\n=== Test Startup ===")
	for name, srv := range cfg.ExternalMCP.Servers {
		if srv.Enabled && !srv.Disabled {
			fmt.Printf("\nAttempting to start %s...\n", name)
			// Note: Actual startup may fail because a real MCP server is required
			err := manager.StartClient(name)
			if err != nil {
				fmt.Printf("  Startup failed (this is normal if no real MCP server exists): %v\n", err)
			} else {
				fmt.Printf("  Startup successful\n")
				// Get client status
