...
// RunCommandRequest executecommandrequest
...
// RunCommandResponse executecommandresponse
	...
	if err != nil {
		...
		if ctx.Err() == context.DeadlineExceeded {
			...
			resp := RunCommandResponse{
				...
				Stderr: se,
				ExitCode: -1,
				Error: "commandexecutetimeout(" + terminalTimeout.String() + ")",
			}
			c.JSON(http.StatusOK, resp)
			return
		}
		h.logger.Debug("commandexecuteexception", zap.String("command", maskTerminalCommand(cmdStr)), zap.Error(err))
	}
