...
// ChatUploadsHandler conversationinuploadattachment(chat_uploads directory)ofmanage API
type ChatUploadsHandler struct {
	logger *zap.Logger
}

// NewChatUploadsHandler createprocessor
...
func (h *ChatUploadsHandler) List(c *gin.Context) {
	...
	if err := os.MkdirAll(root, 0755); err != nil {
		h.logger.Warn("create chat_uploads directoryfailed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	if err != nil {
		h.logger.Warn("conversationattachmentfailed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
