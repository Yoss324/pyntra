...
import (
	...
	openaiClient "pyntra/internal/openai"
...
func (h *FofaHandler) ParseNaturalLanguage(c *gin.Context) {
	...
	if h.cfg == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "configureinitialize"})
		return
	}
	if strings.TrimSpace(h.cfg.OpenAI.APIKey) == "" || strings.TrimSpace(h.cfg.OpenAI.Model) == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "configure AI model:System Settingsin openai.api_key and openai.model(support OpenAI API,such as DeepSeek)",
			"need": []string{"openai.api_key", "openai.model"},
		})
		return
	}
	if h.openAIClient == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "AI initialize"})
		return
	}

	systemPrompt := strings.TrimSpace(`
"FOFA querygeneratedevice/processor".task:inputofsearch, FOFA query.

output():
1) output JSON( markdown//)
2) JSON :
{
 "query": "string,FOFAquery( FOFA orquery)",
 "explanation": "string,optional,such asfield/",
 "warnings": ["string"...] optional,list//needof
}
3) ifinputhas FOFA query(or FOFA ofexpression),"return"is query:
 - field//
 - (),///

query( FOFA ):
- connection:&&(and)/||(or), () expression()
- && and ||(), () ()
- /:
 - = ;field="" ,query"does not existfield"or"is"of
 - == ;field=="" ,query"fieldis"of
 - != ;field!="" ,query"is"of
 - *= ;use * or ? search
- input(field)title/HTML/HTTP/URLfieldinsearch;field(more/more)

field(of,/):
- searchExamples:
 - title="beijing" (= )
 - title=="" (== ,fieldis)
 - title="" (= ,fielddoes not existoris)
 - title!="" (!= ,used foris)
 - title*="*Home*" (*= , * or ?)
...
- (Location):
 - country="CN" or country="in"
 - region="Zhejiang" or region=""(supportinin)
 - city="Hangzhou"
- (Certificate):
 - cert="baidu"
...
generateandnote/attention:
- ,such as title="login"/country="CN"
- :(such as city="beijing" city="BJ"),(such as Beijing/Peking),//
- field(country/region/city)more"output";,, warnings
- does not existof FOFA field; warnings,outputitemsof query
- "itemsand/or", () ,such as:(app="Apache" || app="Nginx") && country="CN"
- or(such as/protocol/port/), query is, warnings needofinformation
`)
	...
	if err := h.openAIClient.ChatCompletion(ctx, requestBody, &apiResponse); err != nil {
		...
		if errors.As(err, &apiErr) {
			h.logger.Warn("FOFAparse:LLMreturnerror", zap.Int("status", apiErr.StatusCode))
			c.JSON(http.StatusBadGateway, gin.H{"error": "AI parsefailed(return 200),checkmodelconfigureorretry"})
			return
		}
		c.JSON(http.StatusBadGateway, gin.H{"error": "AI parsefailed: " + err.Error()})
		return
	}
	if len(apiResponse.Choices) == 0 {
		c.JSON(http.StatusBadGateway, gin.H{"error": "AI returnresult"})
		return
	}
	...
	if err := json.Unmarshal([]byte(content), &parsed); err != nil {
		...
		c.JSON(http.StatusBadGateway, gin.H{
			"error": "AI returnparseis JSON,retryoritems",
			"snippet": snippet,
		})
		return
	}
	...
	if parsed.Query == "" {
		if len(parsed.Warnings) == 0 {
			parsed.Warnings = []string{"needinformation,generateof FOFA query,(such as/port//)."}
		}
	}
...
func (h *FofaHandler) Search(c *gin.Context) {
	...
	if email == "" || apiKey == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "FOFA configure:System Settingsin FOFA Email/API Key,or FOFA_EMAIL/FOFA_API_KEY",
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "FOFA base_url invalid: " + err.Error()})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusBadGateway, gin.H{"error": "request FOFA failed: " + err.Error()})
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		c.JSON(http.StatusBadGateway, gin.H{"error": fmt.Sprintf("FOFA return 2xx: %d", resp.StatusCode)})
		return
	}

	var apiResp fofaAPIResponse
	if err := json.NewDecoder(resp.Body).Decode(&apiResp); err != nil {
		c.JSON(http.StatusBadGateway, gin.H{"error": "parse FOFA responsefailed: " + err.Error()})
		return
	}
	if apiResp.Error {
		msg := strings.TrimSpace(apiResp.ErrMsg)
		if msg == "" {
			msg = "FOFA returnerror"
		}
