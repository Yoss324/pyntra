...
// BatchTaskManager batch task managementdevice/processor
...
// NewBatchTaskManager createbatch task managementdevice/processor
...
func (m *BatchTaskManager) CreateBatchQueue(
	...
	if utf8.RuneCountInString(title) > MaxBatchQueueTitleLen {
		return nil, fmt.Errorf("title %d items", MaxBatchQueueTitleLen)
	}
	if utf8.RuneCountInString(role) > MaxBatchQueueRoleLen {
		return nil, fmt.Errorf("role %d items", MaxBatchQueueRoleLen)
	}
	if len(tasks) > MaxBatchTasksPerQueue {
		return nil, fmt.Errorf("itemsqueue %d task", MaxBatchTasksPerQueue)
	}
...
// ListQueues listqueue(supportfilterandpagination)
func (m *BatchTaskManager) ListQueues(limit, offset int, status, keyword string) ([]*BatchTaskQueue, int, error) {
	...
	if m.db != nil {
		...
		if err != nil {
			return nil, 0, fmt.Errorf("statisticsqueuetotalfailed: %w", err)
		}
		total = count
		queueRows, err := m.db.ListBatchQueues(limit, offset, status, keyword)
		if err != nil {
			return nil, 0, fmt.Errorf("queryqueuelistfailed: %w", err)
		}
		m.mu.Lock()
...
// UpdateQueueStatus updatequeue status
...
func (m *BatchTaskManager) UpdateQueueMetadata(queueID, title, role, agentMode string) error {
	if utf8.RuneCountInString(title) > MaxBatchQueueTitleLen {
		return fmt.Errorf("title %d items", MaxBatchQueueTitleLen)
	}
	if utf8.RuneCountInString(role) > MaxBatchQueueRoleLen {
		return fmt.Errorf("role %d items", MaxBatchQueueRoleLen)
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if queue.Status == BatchQueueStatusRunning {
		return fmt.Errorf("queuein,modify")
	}
...
func (m *BatchTaskManager) UpdateTaskMessage(queueID, taskID, message string) error {
	...
	if !queueAllowsTaskListMutationLocked(queue) {
		return fmt.Errorf("queueexecuteor,edittask")
	}
	for _, task := range queue.Tasks {
		if task.ID == taskID {
			if task.Status == BatchTaskStatusRunning {
				return fmt.Errorf("executeinoftaskedit")
			}
	...
	return fmt.Errorf("taskdoes not exist")
}
func (m *BatchTaskManager) AddTaskToQueue(queueID, message string) (*BatchTask, error) {
	...
	if !queueAllowsTaskListMutationLocked(queue) {
		return nil, fmt.Errorf("queueexecuteor,task")
	}

	if message == "" {
		return nil, fmt.Errorf("taskmessagecannot be empty")
	}
	taskID := generateShortID()
	task := &BatchTask{
		ID: taskID,
	...
	if m.db != nil {
		if err := m.db.AddBatchTask(queueID, taskID, message); err != nil {
			...
			return nil, fmt.Errorf("taskfailed: %w", err)
		}
	}
...
func (m *BatchTaskManager) DeleteTask(queueID, taskID string) error {
	...
	if !queueAllowsTaskListMutationLocked(queue) {
		return fmt.Errorf("queueexecuteor,deletetask")
	}
	taskIndex := -1
	for i, task := range queue.Tasks {
		if task.ID == taskID {
			if task.Status == BatchTaskStatusRunning {
				return fmt.Errorf("executeinoftaskdelete")
			}
			taskIndex = i
			break
		}
