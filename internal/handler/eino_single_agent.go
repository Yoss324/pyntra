...
func (h *AgentHandler) EinoSingleAgentLoopStream(c *gin.Context) {
	...
	h.logger.Info(" Eino ADK single agentrequest",
	...
	sendEvent("progress", "start Eino ADK single agent(ChatModelAgent)...", map[string]interface{}{
		"conversationId": conversationID,
	})
	...
	if h.config == nil {
		sendEvent("error", "serverconfigureload", nil)
		sendEvent("done", "", map[string]interface{}{"conversationId": conversationID})
		return
	}
	...
	if runErr != nil {
		...
		h.logger.Error("Eino ADK single agentexecution failed", zap.Error(runErr))
		taskStatus = "failed"
		h.tasks.UpdateTaskStatus(conversationID, taskStatus)
...
func (h *AgentHandler) EinoSingleAgentLoop(c *gin.Context) {
	...
	h.logger.Info(" Eino ADK single agentrequest", zap.String("conversationId", req.ConversationID))
	...
	if h.config == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "serverconfigureload"})
		return
	}
