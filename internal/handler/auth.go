...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "passwordcannot be empty"})
		return
	}
...
func (h *AuthHandler) Logout(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "hasSign out"})
}
...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "parameterinvalid"})
		return
	}
	...
	if oldPassword == "" || newPassword == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "current passwordandpasswordcannot be empty"})
		return
	}

	if len(newPassword) < 8 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "passwordneed 8 "})
		return
	}

	if oldPassword == newPassword {
		c.JSON(http.StatusBadRequest, gin.H{"error": "passwordandpassword"})
		return
	}

	if !h.manager.CheckPassword(oldPassword) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "current password"})
		return
	}

	if err := config.PersistAuthPassword(h.configPath, newPassword); err != nil {
		if h.logger != nil {
			h.logger.Error("savepasswordfailed", zap.Error(err))
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "savepasswordfailed,retry"})
		return
	}

	if err := h.manager.UpdateConfig(newPassword, h.config.Auth.SessionDurationHours); err != nil {
		if h.logger != nil {
			h.logger.Error("updateauthenticationconfigurefailed", zap.Error(err))
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "updateauthenticationconfigurefailed"})
		return
	}
	...
	if h.logger != nil {
		h.logger.Info("login passwordhasupdate,has")
	}

	c.JSON(http.StatusOK, gin.H{"message": "passwordhasupdate,usepasswordlogin"})
}
...
func (h *AuthHandler) Validate(c *gin.Context) {
	...
	if !ok {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "has"})
		return
	}
