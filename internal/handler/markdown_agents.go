...
func (h *MarkdownAgentsHandler) ListMarkdownAgents(c *gin.Context) {
	if h.dir == "" {
		c.JSON(http.StatusOK, gin.H{"agents": []any{}, "dir": "", "error": "configure agents directory"})
		return
	}
...
func (h *MarkdownAgentsHandler) GetMarkdownAgent(c *gin.Context) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			c.JSON(http.StatusNotFound, gin.H{"error": "filedoes not exist"})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
func (h *MarkdownAgentsHandler) CreateMarkdownAgent(c *gin.Context) {
	if h.dir == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "configure agents directory"})
		return
	}
	...
	if want := agents.WantsMarkdownOrchestrator(filepath.Base(path), body.Kind, string(out)); want {
		...
		if other != "" {
			c.JSON(http.StatusConflict, gin.H{"error": fmt.Sprintf("primary agent definition already exists: %s. Delete it or remove the primary-agent flag first.", other)})
			return
		}
	}
	...
	c.JSON(http.StatusOK, gin.H{"filename": filepath.Base(path), "message": "hascreate"})
}
...
func (h *MarkdownAgentsHandler) UpdateMarkdownAgent(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "hassave"})
}
...
func (h *MarkdownAgentsHandler) DeleteMarkdownAgent(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "hasdelete"})
}
