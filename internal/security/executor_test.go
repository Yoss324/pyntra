...
func TestExecutor_ExecuteInternalTool_QueryExecutionResult(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("saveresultfailed: %v", err)
	}
	...
	if toolResult.IsError {
		t.Fatalf("querysuccessful,backerror: %s", toolResult.Content[0].Text)
	}
	resultText := toolResult.Content[0].Text
	if !strings.Contains(resultText, executionID) {
		t.Errorf("resultID: %s", executionID)
	}
	...
	if toolResult2.IsError {
		t.Fatalf("successful,backerror: %s", toolResult2.Content[0].Text)
	}
	...
	if toolResult3.IsError {
		t.Fatalf("successful,backerror: %s", toolResult3.Content[0].Text)
	}
	
	resultText3 := toolResult3.Content[0].Text
	if !strings.Contains(resultText3, "Port") {
		t.Errorf("result: Port")
	}
	...
	if !toolResult5.IsError {
		t.Fatal("does not existIDbackerror")
	}
}
...
func TestExecutor_ExecuteInternalTool_UnknownTool(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("executing internal toolfailed: %v", err)
	}
	
	if !toolResult.IsError {
		t.Fatal("Unknowntoolbackerror")
	}
	
	if !strings.Contains(toolResult.Content[0].Text, "Unknowntool") {
		t.Errorf("errormessage'Unknowntool'")
	}
}
...
func TestExecutor_ExecuteInternalTool_NoStorage(t *testing.T) {
	...
	if !toolResult.IsError {
		t.Fatal("initializebackerror")
	}
	
	if !strings.Contains(toolResult.Content[0].Text, "resultinitialize") {
		t.Errorf("errormessage'resultinitialize'")
	}
}
...
func TestPaginateLines(t *testing.T) {
	...
	if page.TotalPages != 3 {
		t.Errorf("total pages.: 3, : %d", page.TotalPages)
	}
	if len(page.Lines) != 2 {
		t.Errorf("first page.: 2, : %d", len(page.Lines))
	}
	...
	if page0.Page != 1 {
		t.Errorf("invalidpage1.: %d", page0.Page)
	}
	...
	if len(emptyPage.Lines) != 0 {
		t.Errorf("backresult.: %d", len(emptyPage.Lines))
	}
}
