...
func runDingLoop(ctx context.Context, cfg config.RobotDingtalkConfig, h MessageHandler, logger *zap.Logger) {
	...
	for {
		...
		logger.Info(" Stream connection…", zap.String("client_id", cfg.ClientID))
		err := streamClient.Start(ctx)
		if ctx.Err() != nil {
			logger.Info(" Stream configrestart")
			return
		}
		if err != nil {
			logger.Warn(" Stream connection(/),", zap.Error(err), zap.Duration("retry_after", backoff))
		}
		select {
		case <-ctx.Done():
			return
...
func handleDingMessage(ctx context.Context, msg *chatbot.BotCallbackDataModel, h MessageHandler, logger *zap.Logger) {
	...
	if content == "" {
		logger.Debug("messageis empty,", zap.String("msgtype", msg.Msgtype))
		return
	}
	logger.Info("message", zap.String("sender", msg.SenderId), zap.String("content", content))
	userID := msg.SenderId
	if userID == "" {
	...
	logger.Debug("successful", zap.String("content_preview", reply))
}
