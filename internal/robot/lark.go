...
func runLarkLoop(ctx context.Context, cfg config.RobotLarkConfig, h MessageHandler, logger *zap.Logger) {
	...
	for {
		...
		logger.Info("connectionconnection…", zap.String("app_id", cfg.AppID))
		err := wsClient.Start(ctx)
		if ctx.Err() != nil {
			logger.Info("connectionconfigrestart")
			return
		}
		if err != nil {
			logger.Warn("connection(/),", zap.Error(err), zap.Duration("retry_after", backoff))
		}
		select {
		case <-ctx.Done():
			return
...
func handleLarkMessage(ctx context.Context, event *larkim.P2MessageReceiveV1, h MessageHandler, client *lark.Client, logger *zap.Logger) {
	...
	_, err := client.Im.Message.Reply(ctx, larkim.NewReplyMessageReqBuilder().
		...
		Build())
	if err != nil {
		logger.Warn("failed", zap.String("message_id", messageID), zap.Error(err))
		return
	}
