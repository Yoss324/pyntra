...
func NewEmbedder(ctx context.Context, cfg *config.KnowledgeConfig, openAIConfig *config.OpenAIConfig, logger *zap.Logger) (*Embedder, error) {
	...
	if cfg.Indexing.MaxRPM > 0 {
		...
		if logger != nil {
			logger.Info("knowledge baseenabled", zap.Int("maxRPM", rpm))
		}
	} else if cfg.Indexing.RateLimitDelayMs > 0 {
		rateLimitDelay = time.Duration(cfg.Indexing.RateLimitDelayMs) * time.Millisecond
		if logger != nil {
			logger.Info("knowledge baseenabled", zap.Duration("delay", rateLimitDelay))
		}
	}
	...
	if apiKey == "" {
		return nil, fmt.Errorf("embedding API key config")
	}
...
func (e *Embedder) waitRateLimiter() {
	...
	if e.rateLimiter != nil {
		...
		if err := e.rateLimiter.Wait(ctx); err != nil && e.logger != nil {
			e.logger.Warn("failed", zap.Error(err))
		}
	}
...
func (e *Embedder) EmbedStrings(ctx context.Context, texts []string, opts ...embedding.Option) ([][]float32, error) {
	...
	for attempt := 0; attempt < e.maxRetries; attempt++ {
		if attempt > 0 {
			...
			if e.logger != nil {
				e.logger.Debug("retry", zap.Int("attempt", attempt+1), zap.Duration("wait", wait))
			}
			select {
			case <-ctx.Done():
				return nil, ctx.Err()
		...
		if e.logger != nil {
			e.logger.Debug("failed,retry", zap.Int("attempt", attempt+1), zap.Error(err))
		}
	}
	return nil, fmt.Errorf("retry (%d): %v", e.maxRetries, lastErr)
}
...
func (w *einoFloatEmbedder) GetType() string {
	return "PyntraKnowledgeEmbedder"
}
