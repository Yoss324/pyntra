...
type MarkdownDirLoad struct {
	...
	OrchestratorPlanExecute *OrchestratorMarkdown
	OrchestratorSupervisor *OrchestratorMarkdown
	FileEntries []FileAgent
}
...
func collectMarkdownBasenames(dir string) ([]string, error) {
	...
	if !st.IsDir() {
		return nil, fmt.Errorf("agents: directory: %s", dir)
	}
