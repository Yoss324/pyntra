...
func (db *DB) SaveAttackChainNode(conversationID, nodeID, nodeType, nodeName, toolExecutionID, metadata string, riskScore int) error {
	...
	if err != nil {
		db.logger.Error("saveattack chainfailed", zap.Error(err), zap.String("nodeId", nodeID))
		return err
	}
...
func (db *DB) SaveAttackChainEdge(conversationID, edgeID, sourceNodeID, targetNodeID, edgeType string, weight int) error {
	...
	if err != nil {
		db.logger.Error("saveattack chainfailed", zap.Error(err), zap.String("edgeId", edgeID))
		return err
	}
...
func (db *DB) LoadAttackChainNodes(conversationID string) ([]AttackChainNode, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("queryattack chainfailed: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			db.logger.Warn("scanattack chainfailed", zap.Error(err))
			continue
		}
		...
		if metadataJSON.Valid && metadataJSON.String != "" {
			if err := json.Unmarshal([]byte(metadataJSON.String), &node.Metadata); err != nil {
				db.logger.Warn("parsemetadatafailed", zap.Error(err))
...
func (db *DB) DeleteAttackChain(conversationID string) error {
	...
	if err != nil {
		db.logger.Warn("deleteattack chainfailed", zap.Error(err))
	}
	_, err = db.Exec("DELETE FROM attack_chain_nodes WHERE conversation_id = ?", conversationID)
	if err != nil {
		db.logger.Error("deleteattack chainfailed", zap.Error(err), zap.String("conversationId", conversationID))
		return err
	}
