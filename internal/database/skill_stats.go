...
func (db *DB) SaveSkillStats(skillName string, stats *SkillStats) error {
	...
	if err != nil {
		db.logger.Error("saveSkillsfailed", zap.Error(err), zap.String("skillName", skillName))
		return err
	}
...
func (db *DB) LoadSkillStats() (map[string]*SkillStats, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			db.logger.Warn("loadSkillsfailed", zap.Error(err))
			continue
		}
...
func (db *DB) UpdateSkillStats(skillName string, totalCalls, successCalls, failedCalls int, lastCallTime *time.Time) error {
	...
	if err != nil {
		db.logger.Error("updateSkillsfailed", zap.Error(err), zap.String("skillName", skillName))
		return err
	}
...
func (db *DB) ClearSkillStats() error {
	...
	_, err := db.Exec(query)
	if err != nil {
		db.logger.Error("Skillsfailed", zap.Error(err))
		return err
	}
...
func (db *DB) ClearSkillStatsByName(skillName string) error {
	...
	if err != nil {
		db.logger.Error("skillfailed", zap.Error(err), zap.String("skillName", skillName))
		return err
	}
