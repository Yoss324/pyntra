...
type einoADKRunLoopArgs struct {
	...
	// EmptyResponseMessage placeholder when assistant body text is not captured (multi-agent and single-agent copy differ).
...
func runEinoADKAgentLoop(ctx context.Context, args *einoADKRunLoopArgs, baseMsgs []adk.Message) (*RunResult, error) {
	if args == nil || args.DA == nil {
		return nil, fmt.Errorf("eino run loop: args or Agent is empty")
	}
	if args.McpIDs == nil {
		s := []string{}
		args.McpIDs = &s
	...
	if emptyHint == "" {
		emptyHint = "(Eino session completed, but assistant text output not captured. Please check process details or logs.)"
	}
