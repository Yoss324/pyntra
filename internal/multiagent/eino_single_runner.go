...
// einoSingleAgentName matches ChatModelAgent.Name, used for streaming events to map to main conversation area.
const einoSingleAgentName = "cyberstrike-eino-single"

// RunEinoSingleChatModelAgent uses Eino adk.NewChatModelAgent + adk.NewRunner.Run (official Quick Start's Query also uses Runner API; here history + user message slice equivalent to multi-turn Query).
// Does not replace existing native ReAct; shares runEinoADKAgentLoop SSE mapping and MCP bridge with RunDeepAgent.
func RunEinoSingleChatModelAgent(
	...
	if appCfg == nil || ag == nil {
		return nil, fmt.Errorf("eino single: config or Agent is empty")
	}
	if ma == nil {
		return nil, fmt.Errorf("eino single: multi_agent config is empty")
	}
	...
	return runEinoADKAgentLoop(ctx, &einoADKRunLoopArgs{
		...
		McpIDs:               &mcpIDs,
		DA:                   chatAgent,
		EmptyResponseMessage: "(Eino ADK single-agent session completed, but assistant text output not captured. Please check process details or logs.)",
	}, baseMsgs)
}
