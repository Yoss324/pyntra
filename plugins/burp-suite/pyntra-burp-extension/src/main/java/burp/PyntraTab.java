...
final class PyntraTab implements ITab {
    ...
    private void wireActions() {
        validateButton.addActionListener(e -> {
            ...
            new Thread(() -> {
                try {
                    PyntraClient.Config cfg = currentConfig();
    ...
    PyntraClient.Config currentConfig() {
        String host = hostField.getText().trim();
        String port = portField.getText().trim();
        String password = new String(passwordField.getPassword());
        String baseUrl = "http://" + host + ":" + port;
        PyntraClient.AgentMode mode = agentModeBox.getSelectedIndex() == 1
                ? PyntraClient.AgentMode.MULTI
                : PyntraClient.AgentMode.SINGLE;
        return new PyntraClient.Config(baseUrl, password, mode);
    }
