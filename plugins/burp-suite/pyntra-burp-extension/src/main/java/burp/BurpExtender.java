...
public class BurpExtender implements IBurpExtender, IContextMenuFactory {
    ...
    private PyntraTab tab;
    private final PyntraClient client = new PyntraClient();
    ...
    public void registerExtenderCallbacks(IBurpExtenderCallbacks callbacks) {
        ...
        callbacks.setExtensionName("Pyntra Extension");

        this.tab = new PyntraTab();
        callbacks.addSuiteTab(tab);

        callbacks.registerContextMenuFactory(this);

        callbacks.printOutput("Pyntra extension loaded.");
    }
    ...
    public List<JMenuItem> createMenuItems(IContextMenuInvocation invocation) {
        ...
        JMenuItem sendItem = new JMenuItem("Send to Pyntra (stream test)");
    ...
    private void sendMessage(IHttpRequestResponse msg) {
        if (msg == null) return;
        PyntraClient.Config cfg = tab.currentConfig();
        ...
        if (token == null || token.trim().isEmpty()) {
            JOptionPane.showMessageDialog(tab.getUiComponent(),
                    ...
                    "Pyntra", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        String agentModeStr = (cfg.agentMode == PyntraClient.AgentMode.MULTI) ? "Multi Agent" : "Single Agent";
        String runId = tab.startNewRun(title, agentModeStr, msg);
        tab.appendProgressToRun(runId, "\n[server] " + cfg.baseUrl + "\n\n");

        client.streamTest(cfg, token, prompt, new PyntraClient.StreamListener() {
            @Override
            ...
            public void onError(String message, Exception e) {
                tab.appendProgressToRun(runId, "\n[error] " + message + "\n");
                tab.setRunStatus(runId, "error");
                callbacks.printError("Pyntra stream error: " + message);
                if (e != null) {
